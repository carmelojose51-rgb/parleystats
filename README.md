...
El servidor limita las consultas, restringe el acceso desde orígenes autorizados y no expone detalles internos de errores. Las claves de los proveedores deben configurarse únicamente como variables privadas en Render; nunca deben escribirse en `index.html` ni `app.html`. Las reglas de Firestore quedan versionadas en `firestore.rules` y limitan cada análisis al usuario autenticado que lo creó.
